...
function tmn_iw_enqueue() {
	...
	if (   preg_match('/(?<!\[)\[image_wall.*?\]/', $post->post_content) ) {
		wp_register_script ( 'imagesLoaded', plugins_url( 'imagesloaded.pkgd.min.js'  , __FILE__ ), array(), '4.1.4', true );
		wp_register_script ( 'infinitescroll', plugins_url( 'infinite-scroll.pkgd.min.js'  , __FILE__ ), array('imagesLoaded'), '3.0.6', true );
		wp_enqueue_script  ('tmniwjs', plugins_url( 'image-wall.js'  , __FILE__ ), array('jquery', 'jquery-masonry', 'infinitescroll', 'imagesLoaded'), '3', true);
/*
